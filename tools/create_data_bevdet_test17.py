...
def get_gt(info):
    ...
    Returns:
        ...
        Tensor: GT labels.
    """
    ego2global_rotation = info['cams']['VEHICLE']['ego2global_rotation']  # %%% 本来是CAM_FRONT
...
def add_ann_adj_info(extra_tag):
    ...
    dataroot = '/workspace/data/datasetWYC-mini/'  # %%%
    nuscenes = NuScenes(nuscenes_version, dataroot)
    for set in ['train', 'val']:
        dataset = pickle.load(
            open('/workspace/data/datasetWYC-mini/%s_infos_%s.pkl' % (extra_tag, set), 'rb'))  # %%%
        for id in range(len(dataset['infos'])):
            ...
            dataset['infos'][id]['occ_path'] = \
                '/workspace/data/datasetWYC-mini/gts/%s/%s'%(scene['name'], info['token'])  # %%%
        with open('/workspace/data/datasetWYC-mini/%s_infos_%s.pkl' % (extra_tag, set),  # %%%
                  'wb') as fid:
...
if __name__ == '__main__':
    ...
    root_path = '/workspace/data/datasetWYC-mini/'  # %%%
    extra_tag = 'bevdetv2-nuscenes-test14'  # %%%
    nuscenes_data_prep(
        root_path=root_path,
