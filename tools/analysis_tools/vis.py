...
def check_point_in_img(points, height, width):  # 检查点是否在图像范围内
...
def depth2color(depth):  # 将深度值转化为颜色，用于深度可视化
...
def lidar2img(points_lidar, camrera_info):  # 将雷达点云转化为相机图像坐标系
...
def get_lidar2global(infos):  # 获取雷达到全局坐标系下的转换矩阵
...
def parse_args():
    parser = argparse.ArgumentParser(description='Visualize the predicted '
                                     'result of nuScenes')
    parser.add_argument(
        'res', help='Path to the predicted result in json format')  # 检测结果result.json
    parser.add_argument(
        '--show-range',
        type=int,
        default=50,
        help='Range of visualization in BEV')  # BEV可见范围大小
    parser.add_argument(
        '--canva-size', type=int, default=1000, help='Size of canva in pixel')  # 以像素为单位的画布大小
    parser.add_argument(
        '--vis-frames',
        type=int,
        default=640,
        help='Number of frames for visualization')  # 这咋还对frames有限制？
    parser.add_argument(
        '--scale-factor',
        type=int,
        default=1,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help='Threshold the predicted results')  # 还有阈值，是小于这个置信度的不显示吗？
    parser.add_argument('--draw-gt', action='store_true', default=True)
    # parser.add_argument('--draw-gt', default=False)
    parser.add_argument(
        '--version',
        type=str,
        default='val',
    ...
    parser.add_argument(
        '--fps', type=int, default=20, help='Frame rate of video')  # 调节速率
    parser.add_argument(
...
def main():
    ...
    info_path = \
        args.root_path + '/bevdetv2-nuscenes-test23_infos_%s.pkl' % args.version  # %%%
    dataset = pickle.load(open(info_path, 'rb'))  # 加载pkl
    ...
    for cnt, infos in enumerate(  # infos是val.pkl,按照val.pkl的顺序来布置视频
            dataset['infos'][:min(args.vis_frames, len(dataset['infos']))]):
        if cnt % 10 == 0:
            print('%d/%d' % (cnt, min(args.vis_frames, len(dataset['infos']))))
        # collect instances
        pred_res = res['results'][infos['token']]  # 先找val中第一个sample的token
        pred_boxes = [  # 把该sample中所有实例的位置，尺寸，旋转角度加上
