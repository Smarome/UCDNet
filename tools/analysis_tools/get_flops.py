...
def construct_input(input_shape):
    n = 12
    rot = torch.eye(4).float().cuda().view(1, 1, 4, 4).expand(1,n,4,4)

    intrins = torch.eye(3).float().cuda().view(1,1, 3, 3).expand(1,n,3,3)
    input = dict(img_inputs=[
        torch.ones(()).new_empty((1, n, 3, *input_shape)).cuda(), rot,
        rot, intrins, intrins,
        torch.ones((1, n, 3)).cuda(),
        torch.eye(3).float().cuda().view(1, 3, 3),
        79 * torch.ones((1,n,16,44)).cuda(),
        torch.ones((1,6,16,44)).cuda()
    ])
    return input
