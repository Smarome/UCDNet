...
class ObjectRangeFilter(object):
    ...
    def __call__(self, input_dict):
        ...
        if isinstance(input_dict['gt_bboxes_3d'],
                      (LiDARInstance3DBoxes, DepthInstance3DBoxes)):  # 是LiDARINSTANCE
        ...
        gt_bboxes_3d = gt_bboxes_3d[mask]  # 反正过滤这嘎达就是用pointrange过滤的egopose坐标系下的目标框
        # mask is a torch tensor but gt_labels_3d is still numpy array
        # using mask to index gt_labels_3d will cause bug when
        # len(gt_labels_3d) == 1, where mask=1 will be interpreted
        # as gt_labels_3d[1] and cause out of index error
        gt_labels_3d = gt_labels_3d[mask.numpy().astype(np.bool)]  # 过滤后的

        # limit rad to [-pi, pi]
        gt_bboxes_3d.limit_yaw(offset=0.5, period=2 * np.pi)
        input_dict['gt_bboxes_3d'] = gt_bboxes_3d  # 过滤后的anno
        input_dict['gt_labels_3d'] = gt_labels_3d  # 过滤后的label
