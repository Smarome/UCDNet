...
class NuScenesDataset(Custom3DDataset):
    ...
    def __init__(self,
                 ...
                 ego_cam='UAV_1',  # %%% CAM_FRONT,UAV_1,CAM_BACK_id_2
                 stereo=False):
    ...
    def get_cat_ids(self, idx):  # 原来valid_flag是在这里用的，哦对，样本是基于场景的
    ...
    def get_data_info(self, index):
        ...
        """
        info = self.data_infos[index]  # 原始pkl注释中某一sample的注释
        # standard protocol modified from SECOND.Pytorch
        input_dict = dict(
            sample_idx=info['token'],
            pts_filename=info['lidar_path'],
            sweeps=info['sweeps'],
            timestamp=info['timestamp'] / 1e7,  # %%% 1e6
        )
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv':
                ...
                assert 'bevdet' in self.img_info_prototype  # 到这了
                input_dict.update(dict(curr=info))  # curr来了
                if '4d' in self.img_info_prototype:  # 因为是4d所以找一个临近帧
    ...
    def get_adj_info(self, info, index):
        info_adj_list = []
        adj_id_list = list(range(*self.multi_adj_frame_id_cfg))  # 元组（1，2，1） *是取内容，把外层括号去掉，在range，从1开始，加1,到2-1结束，所以只有1
        if self.stereo:
    ...
    def _evaluate_single(self,
        ...
        # output_dir = "/workspace/BEVDet/debug/"
        rotoo = '/workspace/data/TEST23/'  # %%%
        nusc = NuScenes(
            version=self.version, dataroot=rotoo, verbose=False)  # %%% self.data_root
        eval_set_map = {
            'v1.0-mini': 'mini_val',
            'v1.0-trainval': 'val',
            'v1.16-trainval': 'test16_val',
            'v60-trainval': 'UAV460_val',
            'v1.18-trainval': 'test18_val',
            'v1.19-trainval': 'test19_val',
            'v1.20-trainval': 'test20_val',
            'v1.21-trainval': 'test21_val',
            'v1.23-trainval': 'test23_val'
        } # %%%
        nusc_eval = NuScenesEval(
            ...
            verbose=False)  # self.eval_detection_configs这里设置了检测范围
        nusc_eval.main(render_curves=False)

        # record metrics
        metrics = mmcv.load(osp.join(output_dir, 'metrics_summary.json'))  # 输出里的‘metrics_summary’
        detail = dict()
        metric_prefix = f'{result_name}_NuScenes'
        for name in self.CLASSES:  # 这里确实是两类，是最后打印的东西
