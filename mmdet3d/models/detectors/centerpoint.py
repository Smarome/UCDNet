...
class CenterPoint(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
                          pts_feats,  # [12,256,128,128]
                          gt_bboxes_3d,  # 含12个张量的list
                          gt_labels_3d,  # 含12个张量的list
                          img_metas,  # 里面有sample_idx和雷达路径
        ...
        """
        outs = self.pts_bbox_head(pts_feats)  # ([{'reg': , 'height': , 'dim': , 'rot': , 'vel': , 'heatmap': }])
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs]
        losses = self.pts_bbox_head.loss(*loss_inputs)  # {'task0.loss_xy': ,'task0.loss_z': ,'task0.loss_whl': ,'task0.loss_yaw': ,'task0.loss_vel': , 'task0.loss_heatmap':  }
        return losses
