...
class BEVDet(CenterPoint):
    ...
    def extract_feat(self, points, img, img_metas, **kwargs):
        ...
        img_feats, depth, object_tensor = self.extract_img_feat(img, img_metas, **kwargs)
        pts_feats = None
        return (img_feats, pts_feats, depth, object_tensor)
    ...
    def forward_train(self,
                      ...
                      gt_labels=None,
                      gt_bboxes=None,
                      img_inputs=None,  # 就是result[img_inputs]
                      proposals=None,
        ...
        """
        img_feats, pts_feats, location, object_tensor = self.extract_feat(
            points, img=img_inputs, img_metas=img_metas, **kwargs)  # img_feats[12,256,128,128],location本来是_

        location1 = location[0]  # %%%
        location2 = location[1]  # %%%
        if len(location1) > 0:
            loss_depth = self.loss_fn(location1, location2)
        else:
            loss_depth = torch.tensor(0.0)

        Q = int(object_tensor.shape[0]/2)
        object_tensor_pred = object_tensor[: Q, :, :]
        object_tensor_gt = object_tensor[Q: , :, :]
        loss_object = F.binary_cross_entropy(object_tensor_pred, object_tensor_gt, reduction='mean')

        losses = dict(loss_depth=loss_depth,loss_object=loss_object)  # %%%
        # losses = dict()  # %%%
        losses_pts = self.forward_pts_train(img_feats, gt_bboxes_3d,
                                            gt_labels_3d, img_metas,
                                            gt_bboxes_ignore)  # 该函数来自父类centerpoint
        losses.update(losses_pts)  # 将字典loss_pts添加到字典losses中，形成一个字典，键值融合在一起
        return losses
    ...
    def loss_fn(self, points_1, points_2):  # %%%
        total_loss = 0.0
        for p1, p2 in zip(points_1, points_2):
            total_loss += F.mse_loss(p1, p2)  # %%%
        return torch.sqrt(total_loss/len(points_1))/10
...
class BEVDet4D(BEVDet):
    ...
    def prepare_bev_feat(self, img, rot, tran, intrin, post_rot, post_tran,
                         bda, depth_gd, object_mask, mlp_input):
        x, _ = self.image_encoder(img)
        bev_feat, depth, object_tensor = self.img_view_transformer(
            [x, rot, tran, intrin, post_rot, post_tran, bda, depth_gd, object_mask, mlp_input])  # 改view_transform_core,或者之前的输出加一维，使其传三个出来，包括置信度
        if self.pre_process:
            bev_feat = self.pre_process_net(bev_feat)[0]
        return bev_feat, depth, object_tensor
    ...
    def prepare_inputs(self, inputs, stereo=False):
        # split the inputs into each frame
        B, N, C, H, W = inputs[0].shape  # 数据增强后的图片维，B:BatchSize  N:number  C:channel
        N = N // self.num_frame  # 2，本身N是N*2，现在回到正常的多视角数目了
        imgs = inputs[0].view(B, N, self.num_frame, C, H, W)  # 把两个视角拆开
        imgs = torch.split(imgs, 1, 2)
        imgs = [t.squeeze(2) for t in imgs]
        sensor2egos, ego2globals, intrins, post_rots, post_trans, bda, depth_gd, object_mask = \
            inputs[1:9]  # %%%

        sensor2egos = sensor2egos.view(B, self.num_frame, N, 4, 4)  # 2帧分开
        ego2globals = ego2globals.view(B, self.num_frame, N, 4, 4)  # 2帧分开
        ...
        extra = [
            ...
            depth_gd.view(B, self.num_frame, N, 16, 44)
        ]
        extra = [torch.split(t, 1, 1) for t in extra]
        extra = [[p.squeeze(1) for p in t] for t in extra]
        sensor2keyegos, ego2globals, intrins, post_rots, post_trans, depth_gd = extra
        return imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, \
               bda, depth_gd, object_mask, curr2adjsensor
    ...
    def extract_img_feat(self,
        ...
        bda, depth_gds, object_mask, _ = self.prepare_inputs(img)  # %%% 多加了object_mask
        """Extract features of images."""
        bev_feat_list = []
        depth_list = []
        object_tensor_list = []
        key_frame = True  # back propagation for key frame only
        for img, sensor2keyego, ego2global, intrin, post_rot, post_tran, depth_gd in zip(
                imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, depth_gds):
            if key_frame or self.with_prev:
                ...
                inputs_curr = (img, sensor2keyego, ego2global, intrin, post_rot,
                               post_tran, bda, depth_gd, object_mask, mlp_input)  # %%% 我就把置信度图直接放在这里就行了，就像bda一样
                if key_frame:  # true
                    bev_feat, depth, object_tensor = self.prepare_bev_feat(*inputs_curr)
                else:
                    with torch.no_grad():
                        bev_feat, depth, object_tensor = self.prepare_bev_feat(*inputs_curr)
            else:
            ...
            object_tensor_list.append(object_tensor)  # 我懂了，不用担心非关键帧，反正也不进行反向传播
            key_frame = False
        if pred_prev:
            ...
            for adj_id in range(1, self.num_frame):
                bev_feat_list[adj_id] = \
                    self.shift_feature(bev_feat_list[adj_id],
                                       ...
                                       bda)
        bev_feat = torch.cat(bev_feat_list, dim=1)  # 把关键帧和他前面一帧的bev_feature连起来
        x = self.bev_encoder(bev_feat)
        return [x], depth_list[0], object_tensor_list[0]
...
@DETECTORS.register_module()
class BEVDet4D_original(BEVDet4D):

    def __init__(self,
                 bev_SPE=None,
                 **kwargs):
        super(BEVDet4D_original, self).__init__(**kwargs)
        self.SPE = builder.build_neck(bev_SPE)
