...
class CustomResNet(nn.Module):
    ...
    def __init__(
        ...
        num_channels = [numC_input*2**(i+1) for i in range(len(num_layer))] \
            if num_channels is None else num_channels  # num_channels有值
        self.backbone_output_ids = range(len(num_layer)) \
            if backbone_output_ids is None else backbone_output_ids  # range(0,3)
        layers = []
        if block_type == 'BottleNeck':
            ...
            assert False
        self.layers = nn.Sequential(*layers)  # 在这创建卷积神经网络，pytorch容器
