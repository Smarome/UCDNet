...
class LSSViewTransformer(BaseModule):
    ...
    def __init__(
        ...
        self.depth_relative = depth_relative
        self.input_size = input_size
        self.create_grid_infos(**grid_config)
        self.sid = sid
        self.D = int((depth_relative[1] - depth_relative[0])/depth_relative[2])
    ...
    def create_frustum(self, depth_cfg, input_size, downsample, depth_gd):
        ...
        B, N, H, W = depth_gd.shape  # B,N,H,W
        d = torch.arange(*depth_cfg, dtype=torch.float).view(1, -1, 1, 1).repeat(B, N, 1, H_feat, W_feat).to(depth_gd)
        depth_GD_expanded = depth_gd.unsqueeze(2).repeat(1, 1, self.D, 1, 1)
        result = d * depth_GD_expanded
        if self.sid:
        ...
        x = torch.linspace(0, W_in - 1, W_feat,  dtype=torch.float)\
            .view(1, 1, 1, 1, W_feat).expand(B, N, self.D, H_feat, W_feat).to(depth_gd)
        y = torch.linspace(0, H_in - 1, H_feat,  dtype=torch.float)\
            .view(1, 1, 1, H_feat, 1).expand(B, N, self.D, H_feat, W_feat).to(depth_gd)

        # D x H x W x 3
        return torch.stack((x, y, result), -1)
    ...
    def get_lidar_coor(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
        ...
        frustum = self.create_frustum(self.depth_relative, self.input_size, self.downsample, depth_gd)
    ...
    def view_transform_core(self, input, depth, tran_feat, depth_gd):
        ...
        if self.accelerate:
            ...
            coor = self.get_lidar_coor(*input[1:7], depth_gd)
            bev_feat = self.voxel_pooling_v2(
                coor, depth.view(B, N, self.D, H, W),
                tran_feat.view(B, N, self.out_channels, H, W))
        return bev_feat, depth, 1

    def view_transform(self, input, depth, tran_feat, depth_gd):
        if self.accelerate:
            self.pre_compute(input)
        return self.view_transform_core(input, depth, tran_feat, depth_gd)
    ...
    def forward(self, input):
        ...
        x = x.view(B * N, C, H, W)
        x = self.depth_net(x)
        depth_gd = input[7].to(x)  # %%%

        depth_digit = x[:, :self.D, ...]
        tran_feat = x[:, self.D:self.D + self.out_channels, ...]
        depth = depth_digit.softmax(dim=1)
        return self.view_transform(input, depth, tran_feat, depth_gd)
