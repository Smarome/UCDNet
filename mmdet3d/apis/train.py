...
def train_detector(model,
    ...
    runner_type = 'EpochBasedRunner' if 'runner' not in cfg else cfg.runner[
        'type']
    data_loaders = [
        build_mmdet_dataloader(  # 6.1 初始化data_loaders,内部会初始化GroupSampler
            ds,
            cfg.data.samples_per_gpu,
            cfg.data.workers_per_gpu,
    ...
    # put model on gpus
    if distributed:  # 6.2 基于是否使用分布式训练，初始化对应的 DataParallel
        find_unused_parameters = cfg.get('find_unused_parameters', False)
        # Sets the `find_unused_parameters` parameter in
        # torch.nn.parallel.DistributedDataParallel
        model = MMDistributedDataParallel(
    ...
    # build runner
    optimizer = build_optimizer(model, cfg.optimizer)  # 优化器初始化
    ...
    runner = build_runner(  # 6.3 初始化 runner
        cfg.runner,
        default_args=dict(
            model=model,
            optimizer=optimizer,
    ...
    # register hooks
    runner.register_training_hooks(  # 6.4 注册必备hook
        cfg.lr_config,
        optimizer_config,
    ...
    if validate:
        ...
        # In this PR (https://github.com/open-mmlab/mmcv/pull/1193), the
        # priority of IterTimerHook has been modified from 'NORMAL' to 'LOW'.
        runner.register_hook(  # 6.6 注册用户自定义hook
            eval_hook(val_dataloader, **eval_cfg), priority='LOW')
    ...
    if cfg.resume_from:  # 6.7 权重的恢复和加载
        runner.resume(cfg.resume_from)
    elif cfg.load_from:
        runner.load_checkpoint(cfg.load_from)
    runner.run(data_loaders, cfg.workflow)  # 6.8 开始训练
